...
def spending_entropy(row: pd.Series):
...
def create_credit_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["revolve_percentage_rank"] = (
        df["revolve_percentage"].rank(pct=True)
    )
...
def create_reward_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Create reward behaviour features.
    """

    df = df.copy()

    # ---------------------------------------------
    # Remaining reward balance (breakage proxy)
    # ---------------------------------------------
    df["reward_breakage"] = (
        df["f4"] - df["f21"]
    ).clip(lower=0)

    ENGINEERED_FEATURES["reward"].append(
        "reward_breakage"
    )
    ...
    # ---------------------------------------------
    # Reward earned per dollar spent
    # ---------------------------------------------
    df["reward_per_spend"] = (
        df["f4"] /
        (df["f5"] + EPSILON)
    ...
    # ---------------------------------------------
    # Reward redeemed per dollar spent
    # ---------------------------------------------
    df["redeem_per_spend"] = (
        df["f21"] /
        (df["f5"] + EPSILON)
    ...
    df["has_reward_redemption"] = (
        df["f21"] > 0
    ).astype("int8")

    ENGINEERED_FEATURES["reward"].append(
        "has_reward_redemption"
    )
    rank_cols = [
    "reward_breakage",
    "reward_redemption_rate",
    ...
    for col in rank_cols:
        df[f"{col}_rank"] = df[col].rank(pct=True)
    print(
        f"Created {len(ENGINEERED_FEATURES['reward'])} reward features."
    )
...
def create_behavior_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["category_usage_count"] = (
        df[spend_cols] > 0
    ).sum(axis=1)

    ENGINEERED_FEATURES["behavior"].append(
    ...
    df["entertainment_affinity"] = (
        df["f9_ratio"]
    )

    ENGINEERED_FEATURES["behavior"].append(
        "entertainment_affinity"
    )
    ...
    print(
        f"Created {len(ENGINEERED_FEATURES['behavior'])} behavior features."
    )
